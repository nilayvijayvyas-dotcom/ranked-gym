// dashboard.js
// Clean, defensive dashboard renderer for GymRank.
...
function renderDashboard() {
  ...
  // Get workouts + muscle stats from your state layer (these functions should exist)
    ...
    const cardsArr = trainedMuscles.map((muscle) => {
      ...
      try {
        if (typeof getRank === 'function') {
          const r = getRank(pb);
          // normalize fields needed; r may be string or object
        ...
        // ignore and use default rank
      }

      // Try to compute next rank threshold and progress.
      // We will attempt to use a global thresholds array if available (rankThresholds)
      let toNext = 'Max rank!';
      let progressPct = 100;
      try {
        ...
        if (thresholds && Array.isArray(thresholds)) {
          // find current rank index by rank.name (case-insensitive)
          ...
          // fallback: if rank has .min and there is a next via value
      ...
      // Build card HTML (safe escaped insertions are assumed; values are simple)
