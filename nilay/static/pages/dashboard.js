function renderDashboard() {
  ...
  try {
    ...
    if (dateStrings.length > 0) {
      let expected = new Date(dateStrings[0]); 
      streak = 0;
  ...
  // Recent activity, which will be last 5 workouts logged
  const recent = Array.isArray(workouts) ? workouts.slice(-5).reverse() : [];

  // Build muscle cards for muscles that have at least one workout logged
  let muscleCardsHtml = '';
  try {
    // find muscles that were trained 
    ...
    const cardsArr = trainedMuscles.map((muscle) => {
      ...
      // Attempt to get rank info from getRank() if thats available
